...
- **Version badge**: V1.30 in bottom-right corner on every page
...
## Version History
- **V1.30**: Completed the RSA/ECC multi-key-type support across the frontend. Updated encrypt.html key generator and encrypt dropdowns to include RSA-3072, ECC-P256, ECC-P384, and ECC-P521 (in addition to existing RSA-2048/4096). JS now sends `key_type` string parameter instead of bare integer `key_size`. Updated pgp-practice.html encrypt/decrypt helper dropdown with all key types. Added Tim bot key type selector: when Tim is selected, a dropdown lets users pick any of the 6 key types (RSA-2048/3072/4096, ECC-P256/P384/P521). The `/api/practice/key` endpoint accepts `key_type` param for Tim and generates/loads the matching keypair. The `/api/practice/send` endpoint uses the selected key type for decryption. `init_practice_keys()` now generates Tim keypairs for all 6 types at startup. Added a Key Size Speed & Security Comparison visualizer card to the Practice hub page: a table showing each key type's security level (bits), relative speed (colored bars), and best use case, with a summary explaining why ECC is faster than RSA at equivalent security. Bumped version badge to V1.30.
