...
- **Version badge**: V1.31 in bottom-right corner on every page
...
## Version History
- **V1.31**: Restyled the left sidebar navigation links across all 13 pages for better spacing and visual clarity. Removed old purple base styles (rgba purple backgrounds, uppercase text, tight 0.6rem gap). New CSS: `display: block` links with 14px vertical padding, 4px margin between links, bold text, 8px border radius, blue gradient active state with inset border, and a hover effect that slides links right by 3px. Links are now spacious, easy to click, and match the light blue/white site theme. Bumped version badge to V1.31.
