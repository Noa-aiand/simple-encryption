...
- **Version badge**: V1.32 in bottom-right corner on every page
...
## Version History
- **V1.32**: Reorganized learn.html into Beginner / Intermediate / Advanced difficulty groups. Tabs are now ordered from beginner to advanced with visible group labels. Moved Key Size Speed & Security Comparison visualizer from practice.html to the Key Sizes & Algorithms tab on learn.html (was out of place on the practice hub). Added new "Air-Gap Devices" advanced tab covering air-gapped PGP primary key management, setting up an air-gapped machine, air-gap attacks (USB malware, acoustic, TEMPEST), and hardware security keys (YubiKey) as a middle-ground option. The Advanced OPSEC tab emphasizes that user error is the #1 threat regardless of encryption strength, with a "build OPSEC as a habit" framework, common mistakes (nothing to hide, convenience over security, security theater, all-or-nothing thinking), and the OPSEC mindset (assume breach, least privilege, defense in depth, verify don't trust, compartmentalize). Bumped version badge to V1.32.
