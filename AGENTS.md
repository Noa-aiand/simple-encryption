...
- **Version badge**: V1.27 in bottom-right corner on every page
...
## Version History
- **V1.27**: Added "Build Your Own Threat Model" interactive section to the threat-model.html practice page. Users fill in four fields (what they are protecting, who the adversary is, what happens if they fail, and one action they can take today), then click "Generate Threat Model" to get a personalized summary with context-aware guidance tips (detects keywords like passwords, email, PGP, cloud, social media, identity documents, personal adversaries, etc. and provides specific OPSEC advice). Includes copy-to-clipboard and clear buttons. Also rewrote the Social Engineering Red Flags quiz: flag options are now neutral observations ("the message says the password expires in 2 hours") instead of giving away the answer ("Urgency / time pressure"), added 4 new scenarios (fake package delivery, IT helpdesk with bypass instructions, Netflix typosquat with number 1, compromised coworker account), bringing the total to 10 scenarios. Added selected-item checkbox indicator (blue filled box with checkmark) on the social engineering quiz. Bumped version badge to V1.27.
